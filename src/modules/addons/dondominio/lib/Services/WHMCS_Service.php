...
class WHMCS_Service extends AbstractService implements WHMCSService_Interface
{
    ...
    /**
     * Retrieves domain suggestions from API and returns them with prices associated
     *
     * @param string $text Text to get similar suggestions
     *
     * @return array Suggestions with prices
     */
    public function getDomainSuggestions($text)
    {
        ...
        $suggestions = $this->getApiServiceForDomainSuggestions()->getDomainSuggestions($text);
        ...
        foreach ($results as $sld => $result) {
            foreach ($result as $tld => $available) {
                ...
                $domainPricing = $this->getDomainPricing(['extension' => '.' . $tld]);

                if (is_null($domainPricing)) {
                    continue;
                }

                $pricing = Capsule::table('tblpricing')->selectRaw("
                    msetupfee AS 1Y,
                    qsetupfee AS 2Y,
                    ssetupfee AS 3Y,
                    asetupfee AS 4Y,
                    ...
                    annually AS 9Y,
                    biennially AS 10Y,
                    ( SELECT prefix FROM tblcurrencies WHERE id = '" . $currency . "' ) AS currency_prefix,
                    ( SELECT suffix FROM tblcurrencies WHERE id = '" . $currency . "' ) AS currency_suffix
                ")->where(['type' => 'domainregister', 'currency' => $currency, 'relid' => $domainPricing->id])->first();
